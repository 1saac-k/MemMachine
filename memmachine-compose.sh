...
set_provider_api_keys() {
    ...
    if [ -f ".env" ]; then
        ...
        if [[ "$llm_model" == "openai_model" ]] || [[ "$embedder_model" == "openai_embedder" ]]; then
            if grep -q "<YOUR_API_KEY>" configuration.yml; then
                ...
                if [[ $reply =~ ^[Yy]$ ]]; then
                    ...
                    # Auto-populate with EMPTY to prevent runtime errors
        ...
        if [[ "$llm_model" == "aws_model" ]] || [[ "$embedder_model" == "aws_embedder_id" ]]; then
            if grep -q "<AWS_ACCESS_KEY_ID>" configuration.yml || grep -q "<AWS_SECRET_ACCESS_KEY>" configuration.yml; then
                ...
                if [[ $reply =~ ^[Yy]$ ]]; then
                    ...
                    print_warning "AWS credentials left as placeholders. Update configuration.yml before running."
                fi
            else
        ...
        if [[ "$llm_model" == "ollama_model" ]] || [[ "$embedder_model" == "ollama_embedder" ]]; then
            print_prompt
            read -p "Ollama LLM base URL [http://host.docker.internal:11434/v1]: " base_url
            base_url=${base_url:-http://host.docker.internal:11434/v1}

            safe_sed_inplace "/ollama_model:/,/base_url:/ s|base_url: .*|base_url: \"$base_url\"|" configuration.yml
            print_success "Set Ollama LLM base URL: $base_url"

            local embedder_base_url=""
            print_prompt
            read -p "Use a different base URL for Ollama embedding? (y/N) " reply
            if [[ $reply =~ ^[Yy]$ ]]; then
                print_prompt
                read -p "Ollama embedding base URL [${base_url}]: " embedder_base_url
                embedder_base_url=${embedder_base_url:-$base_url}
            else
                embedder_base_url="$base_url"
            fi

            safe_sed_inplace "/ollama_embedder:/,/base_url:/ s|base_url: .*|base_url: \"$embedder_base_url\"|" configuration.yml
            print_success "Set Ollama embedding base URL: $embedder_base_url"
        fi
    fi
}
    ...
    if [ -f ".env" ]; then
        ...
        if [[ "$llm_model" == "openai_model" ]] || [[ "$embedder_model" == "openai_embedder" ]]; then
            if [ -z "$OPENAI_API_KEY" ] || [ "$OPENAI_API_KEY" = "your_openai_api_key_here" ]; then
                ...
                print_info "OPENAI_API_KEY is set to 'EMPTY' (no authentication)"
            else
